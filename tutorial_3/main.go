...
func removeEntry(w http.ResponseWriter, r *http.Request) (interface{}, *handlerError) {
	entry := mux.Vars(r)["entry"]
	e := os.Remove(blog + entry)
	if e != nil {
		return nil, &handlerError{e, "Could not find entry", http.StatusNotFound}
	}
	return make(map[string]string), nil
...
func main() {
	...
	router.Handle("/blog/{entry}", handler(removeEntry)).Methods("DELETE")
	http.Handle("/", router)
